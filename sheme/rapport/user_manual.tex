...
This manual describes how to install the git-remote-mediawiki test environment on a machine with git installed on it.
\part*{Prerequisite}
\section{Get the sources}
To be able to run this test environment you first need to get the sources developped in our project.

To do so, see the following git-hub: \lstinline!https://github.com/Fafinou/git/!.\\
Then, get the Master branch to apply our main five patches. You are now able to run our git-remote-mediawiki test environment.
\section{Packages required}
In order to run this test environment correctly, you will need the following packages.
\begin{itemize}
\item php5-cli
\item lighttpd
\item php5-cgi
...
Once you have all the prerequisite, you are able to use the git-remote-mediawiki test environment.
First, you need to install a wiki on which you can test your future modification to git-remote-mediawiki.
To do so, execute the following.
\begin{itemize}
\item go into the \lstinline!contrib/mw-to-git/t/! directory
\item Edit the file \lstinline!test.config!. You may choose several installation parameters.
\item run \lstinline!./install-wiki.sh install!
\item check on your favourite web browser if your wiki is correctly installed
\end{itemize}
